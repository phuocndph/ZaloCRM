...
import { ContextBuilderError, IntentEngineError, analyzeConversationIntent } from './intent-engine-service.js';
...
function fail(reply: FastifyReply, error: unknown) { if (error instanceof IntentEngineError || error instanceof ContextBuilderError) return reply.status(error.statusCode).send({ error: error.message, code: error.code }); const normalized = AIErrorHandler.normalize(error); return reply.status(normalized.statusCode).send({ error: normalized.message, code: normalized.code }); }
export async function intentEngineRoutes(app: FastifyInstance) { app.post('/api/v1/ai/intent/conversations/:conversationId/analyze', { preHandler: [settingsEdit, requireAdmin] }, async (request, reply) => { try { const privacy = await buildPrivacyContext(request); const body = (request.body ?? {}) as { modelConfigId?: string; maxTokens?: number }; return await analyzeConversationIntent({ orgId: request.user!.orgId, userId: request.user!.id, role: request.user!.role, privacyUnlocked: privacy.privacyUnlocked }, (request.params as { conversationId: string }).conversationId, body); } catch (error) { return fail(reply, error); } }); }
