...
export async function rotateRefreshToken(
  ...
  // Đã xoay rồi (usedAt set).
  if (rec.usedAt) {
    const sinceUsed = Date.now() - rec.usedAt.getTime();
    if (sinceUsed <= config.refreshGraceMs) {
      // Race đa tab benign: cấp token mới cùng family, KHÔNG revoke.
  ...
  // Đường bình thường: cấp token mới cùng family + đánh dấu token cũ đã dùng.
  const issued = await issueRefreshToken(rec.userId, meta, rec.familyId);
  await runSystemQuery(() =>
